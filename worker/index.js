...
Многошаговое управление устройством:
- Выполняй задачу до достижения конечной цели пользователя, а не только первый шаг.
- Для действий с Android-интерфейсом вызывай НЕ БОЛЕЕ ОДНОГО device tool за один ответ модели.
- После каждого device tool дождись его результата и только затем выбирай следующий шаг.
- Если после действия returned screen недостаточно для уверенного продолжения, следующим шагом вызови get_screen_state.
- Не планируй клики по элементам, которые появятся только после предыдущего действия, пока не получен обновлённый экран.
- Если нужного элемента не видно, безопасно используй scroll_screen и снова оцени полученный экран.
